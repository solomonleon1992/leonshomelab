...
|---|---|---|---|
| `caddy` | `caddy:2-alpine` | 80, 443 | ✅ **Ansible** — `ansible/caddy.yml` (volumes declared `external`) |
| `portainer` | `portainer/portainer-ce:latest` | **9443** (9000 exposed, *not* published) | ✅ **Ansible** — `ansible/portainer.yml` |
| `jellyfin` | `jellyfin/jellyfin:latest` | `network_mode: host` — binds host interfaces directly (8096 and DLNA/discovery ports). Docker reports no mappings by design; 8096 is reachable | ✅ **Ansible** — `ansible/jellyfin.yml` |
| `DVWA` | `vulnerables/web-dvwa:latest` | 8080 | ✅ **Ansible** — `ansible/dvwa.yml` |
| `secplus-drill` | `nginx:alpine` | 8088 | ✅ **Ansible** — `ansible/secplus-drill.yml` (compose file is generated, digest-pinned) |
...
**VM 102 also bypasses Pi-hole — undocumented until 2026-08-20.** Its `/etc/resolv.conf` contains a single `nameserver 8.8.8.8`, so `Docker-Host` cannot resolve any lab name: `getent hosts n8n.leonshomelab.local` returns nothing there, while Pi-hole answers `192.168.0.25` correctly for anyone who asks it. Caddy is unaffected because its upstream is addressed by IP, but it means **the host running the reverse proxy cannot reach the site it serves by name**, and any future service on VM 102 that resolves a lab hostname will fail. Whether this is deliberate (matching the Proxmox/OPNsense rationale above) or drift is unresolved — it was never recorded either way.
...
| `N8N_ENCRYPTION_KEY` | **Exactly one file on VM 105** — `~/n8n-stack/docker-compose.yml`; **copy in Bitwarden (2026-08-12)** | ✅ Backed up. Five stale plaintext copies removed 2026-08-20 (§5.5) |
...
### 5.5 Plaintext secret copies — swept 2026-08-20

A permissions question turned into an audit. `N8N_ENCRYPTION_KEY` existed in **six** files on VM 105, five of them stale backups accumulated over four weeks of ordinary changes:

```
docker-compose.yml.bak-20260814          docker-compose.yml.pre-oauth
docker-compose.yml.bak-20260820          docker-compose-backup-20260721%.yml
docker-compose.yml.backup-20260728-…     (+ docker-compose.yml.ansible-bak)
```

All were verified to carry the same variable set as the live file before deletion — nothing unique was lost. **One file now holds the key**, which is the irreducible minimum until the `.env` migration.

`~/docker-compose-backup-20260721%.yml` is the instructive one: a month old, mode `664`, and **outside the stack directory**, so every check scoped to `~/n8n-stack/` missed it. Backups of secret-bearing files are created casually during ordinary work and then never revisited; each one is a permanent copy of a Tier-0 secret with no expiry and nothing tracking it.

**The file mode was a red herring.** `/home/leon` is `0750`, so "other" cannot traverse into it and the `664` on the file never came into play. The only login shells on the host are `root` and `leon`. Tightening to `0600` would have been theatre; deleting five plaintext copies was the actual improvement.

**No local rollback copies are kept.** Tier 1 holds the compose file encrypted, off-host, with 14 versions of history — strictly better than a plaintext copy on the same disk as the thing it protects. `ansible/n8n.yml` no longer writes one.
...
| R-15 | **Now real (2026-08-20).** The Ansible control node concentrates **lab-wide root access into a single SSH key**, held on the MSI Katana where BitLocker is off (R-14). A passphrase-less key on an unencrypted disk would be equivalent to storing root credentials for every host in plaintext. Logged before the key existed so the controls preceded it, which is what happened | Medium | **Mitigated 2026-08-20** — key created passphrase-protected (**verified**: an empty passphrase is rejected), held in `ssh-agent` on a fixed socket, dedicated and separate from interactive admin keys so it rotates independently, `600` perms. Fingerprint `SHA256:QOuFJwWk…`. **Outstanding: confirm the passphrase is in Bitwarden, not Vaultwarden** (§7.11) |
...
**Hardened 2026-08-20 — the risk is unchanged, the likelihood is lower.** This acceptance rested on losing `caddy_caddy_data` being unlikely. It was less unlikely than it looked: the compose file declared the volumes **non-external**, as `caddy_data` and `caddy_config`, so Docker derived their real names from the compose project name — which comes from the **directory name**. Renaming `~/docker/caddy` would have produced fresh empty volumes, a brand-new root CA, and a re-trust on every client, with no error at any point.

They are now declared `external: true` under their absolute names. Verified from a scratch directory with a deliberately different project name: the compose still resolved to `caddy_caddy_data`. If the volumes are ever missing, `ansible/caddy.yml` refuses to start rather than letting Caddy quietly generate a new authority, and it compares the root CA fingerprint before and after every run.

That assertion matters because the failure is invisible: with a new CA, Caddy still issues leaf certs, still serves HTTPS, and still returns 200 — while every existing client rejects it.
...
**One gap recorded rather than hidden, and one corrected.** Pi-hole's `gravity.db` is 63 MB of downloaded blocklist content that `pihole -g` regenerates, and stays excluded. The **adlist URLs** inside it — which nothing else records, and which are what `pihole -g` regenerates *from* — are now captured (2026-08-20).

> **Correction.** This section previously stated the URLs could not be extracted because `sqlite3` is absent on the Pi. That was wrong. A standalone `sqlite3` binary is indeed absent, but **pihole-FTL bundles it**, and `pihole-FTL sqlite3` queries `gravity.db` directly without sudo. The capability existed the whole time; the check looked for the wrong thing and the conclusion was recorded as fact.
...
A third pass on **2026-08-20** was the first performed by machine rather than by hand: `ansible all -m ansible.builtin.setup` against all six hosts, read-only, exit code 0. Every CPU, memory and OS figure it returned **matched §4** — Docker-Host 2 vcpu / 3911 MB, n8n-ai-stack and Wazuh-SIEM 4 vcpu / 7937 MB each, OPNsense FreeBSD 14.3-RELEASE-p8 / 1951 MB, `pve` 32006 MB. Two details this document had recorded only loosely are now exact: Pi-hole runs **Debian 13.4** on kernel `6.12.75+rpt-rpi-v8`, and the Proxmox host is **Debian 13.4** on `6.17.2-1-pve`.

This matters beyond the numbers. Until now every figure here rested on commands someone ran once and transcribed; it can now be re-checked in seconds, by anyone, without trusting the transcription.
...
| 2026-08-20 | **Phase 1 of the IaC migration complete — the lab is now managed from code, read-only so far.** WSL2 + Ubuntu 26.04 LTS on the MSI Katana, Ansible 13.1.0 (`ansible-core` 2.20.1), a **dedicated passphrase-protected** ed25519 key distributed to all six hosts, and `ansible/inventory.yml` + `ansible/ansible.cfg` committed. **Exit criterion met: 6/6 hosts answered `ansible all -m ansible.builtin.setup`, exit code 0, nothing changed.** **R-15 moves from anticipated to real and mitigated** — the key exists, and its passphrase protection was *verified* rather than assumed (an empty passphrase is rejected); it is held in `ssh-agent` on a fixed socket, separate from interactive admin keys. **§9 gains a machine-verified pass** — every CPU/RAM/OS figure returned matched §4, the first corroboration of this document by something other than hand-transcribed commands, and Pi-hole and Proxmox OS versions are now exact. Four traps recorded in `IaC-MIGRATION.md` §5 so they are not rediscovered: `wsl --install` is two-stage; Ubuntu's `.bashrc` returns early for non-interactive shells so agent config belongs in `.profile`; OPNsense root's tcsh returns *"Illegal variable name"* on a **successful** key test, which is not an auth failure; and Ansible ignores `ansible.cfg` in a world-writable directory, which `/mnt/c` is — so the repo copy is a recovery artifact and `~/.ansible.cfg` is operative. |
| 2026-08-20 | **First service converted to Ansible — `secplus-drill`** (`IaC-MIGRATION.md` §6, position 1). Chosen because it is the cheapest possible thing to get wrong: one container, one port, no database, no credentials, zero dependents. Converged, then re-run to prove **`changed=0`** — idempotency is the actual pass criterion, since a playbook that reports changes on every run is describing a moving target rather than a desired state. The compose file is now generated by the playbook with `nginx` **pinned by digest**, so this service no longer floats. **Check mode paid for itself immediately**, catching a defect that would have failed every dry run: the HTTP-verification task skips under `--check`, so the report task referenced a variable that did not exist. Fixed, and the docker precondition now carries `check_mode: false` — a precondition skipped during a dry run is not a precondition. **A backup gap was found in passing:** Tier 1 captured `~/secplus-drill/docker-compose.yml` but **not `html/index.html`** — 44 KB of hand-written SY0-701 study material existing in exactly one place. The backup was preserving the eight-line compose file, which the playbook can now regenerate, while missing the content the service exists to serve. Added; **30 items**. The playbook deliberately does **not** manage the html directory's contents: if the source were ever missing, one run would delete them, and a first conversion is not where to hand a playbook that power. |
| 2026-08-20 | **DVWA and Portainer converted to Ansible** (`IaC-MIGRATION.md` §6, position 2) — `ansible/dvwa.yml` and `ansible/portainer.yml`, both `changed=0` on re-run. The repo's compose files are now the deployed source rather than a copy of it: **they had already drifted**, having gained explanatory comments after the 2026-08-14 deployment, so host and repo differed by content though not by meaning. Verified comments-only before overwriting. That silent divergence is precisely what these playbooks now prevent. **Portainer's playbook asserts its own data survived** — `GET /api/users/admin/check` returning 204 proves an admin user exists, where 404 would mean an empty database. It also **refuses to create `portainer_data` if absent**: on an existing host a missing volume means something is wrong, and on a fresh one it means data has not been restored yet — silently creating an empty volume would bring Portainer up looking healthy with every user and endpoint gone, which is the failure mode hardest to notice and most expensive to find late. |
| 2026-08-20 | **Jellyfin converted to Ansible** (`IaC-MIGRATION.md` §6, position 3) — `ansible/jellyfin.yml`, `changed=0` on re-run, 10.11.6 unchanged (the digest matched the one recorded 2026-08-14, so pinning was a freeze). The obsolete `version: "3"` key was dropped. **A finding worth acting on separately: `/mnt/media` is an empty plain directory, not a mountpoint — Jellyfin is currently serving nothing.** Either media was never added or a mount is gone; the playbook reports it on every run rather than passing silently. **The playbook asserts the config and media paths rather than creating them.** Creating `/mnt/media` would mask a failed mount — the bind would succeed, Jellyfin would start cleanly, and the library would simply be empty; creating `config/` would present a first-run setup wizard as though the server had never been configured. Both are failure modes that look like health. **Jellyfin's config added to Tier 1** — users, library metadata and watch history, 664 KB on disk and ~28 KB compressed. A rescan rebuilds metadata; nothing rebuilds who watched what. It was absent while the compose file beside it — now regenerable from the playbook — was captured. `cache/` is deliberately excluded as regenerable. **31 items.** |
| 2026-08-20 | **Caddy converted to Ansible** (`IaC-MIGRATION.md` §6, position 4) — `ansible/caddy.yml`, `changed=0` on re-run, v2.11.4 unchanged. The Caddyfile now lives in the repository and is deployed from it, which makes the §7.8 cutover a one-line edit plus a playbook run. **The real work was hardening the volumes** (§7.9): they were declared *non-external*, so their true names derived from the compose project name, which derives from the directory name — renaming `~/docker/caddy` would have generated a new root CA and required a re-trust on every client, silently. Now `external: true` under absolute names, verified from a scratch directory with a different project name that still resolved to `caddy_caddy_data`. **The playbook compares the root CA fingerprint before and after every run and fails if it changed**, because that failure is otherwise invisible — a new CA still issues leaf certs, still serves HTTPS, still returns 200, and every existing client rejects it. Verified end to end: fingerprint `46:0A:79:F7…` identical before and after, and `https://n8n.leonshomelab.local/` returns 200 from the workstation with **certificate validation on**. **Second finding: VM 102 bypasses Pi-hole**, resolving via `8.8.8.8` — recorded in §5.2. The host running the reverse proxy cannot resolve the site it serves, so the playbook forces SNI to loopback rather than depending on DNS. |
| 2026-08-20 | **Pi-hole converted to Ansible as a *verification* playbook** (`IaC-MIGRATION.md` §6 position 5, §8.1) — `ansible/pihole.yml` asserts and writes nothing. `pihole.toml` carries the header *"This file is managed by pihole-FTL"*, and templating it would recreate the OPNsense `authorized_keys` problem: Ansible silently reverting every web-UI change on a schedule. The playbook checks FTL is active **and enabled**, that both expected local DNS records exist, that a lab name resolves to 192.168.0.25, and that public resolution still works — *configuration looks right* and *DNS actually works* are different claims, and only the second is what a household notices. Tested by feeding it a deliberately absent record: it failed, which is the only way to know an assertion is not passing vacuously. **Two corrections came out of this.** First, `sqlite3` **is** available on the Pi — bundled inside `pihole-FTL` — so the adlist URLs are now captured in Tier 1 (**32 items**); the earlier claim that they were unextractable checked for a standalone binary and recorded the conclusion as fact. Second, and more seriously: **`Backup-Tier1.ps1` could report success while an item failed.** `(… ).Count` returns `$null` when exactly one item matches in PowerShell 5.1, so `$failCount -gt 0` was `False` and the run printed "All N items captured" and exited 0. Two or more failures counted correctly; **exactly one was invisible** — the common case, hidden, latent since 2026-08-12. Both counts are now wrapped in `@()`. |
| 2026-08-20 | **n8n converted — Phase 2 of the IaC migration complete, all six services now managed by Ansible** (`IaC-MIGRATION.md` §6 position 6). `ansible/n8n.yml` makes one surgical edit — declaring the three volumes **external** — and never reads, writes or transports a secret: `~/n8n-stack/docker-compose.yml` holds `N8N_ENCRYPTION_KEY` inline, so the playbook deliberately does not deploy it from the repository. It asserts that workflow, credential and `job_matches` counts are identical before and after, and that **credentials still decrypt** (`export:credentials --decrypted`, exit code only, output deleted unread) — the only check that proves the encryption key survived, because workflows restore from plaintext JSON and look perfect with a wrong key. The volumes carried the same latent fault as Caddy: project-prefixed names derived from the directory name, so renaming `~/n8n-stack` would have yielded an empty PostgreSQL with no n8n database and no `jobtracker`. **A mistake worth recording: the first version of the volume edit used a loose regular expression that, on a second run, matched a fragment of the already-converted block and appended a duplicate — leaving the live compose file unparseable.** The stack kept running only because the containers were already up. Repaired, and the task now carries two independent guards: a pattern that matches only the original bare form, and a skip once three external declarations exist. Verified `changed=0` across three consecutive runs, containers never restarted, and the 60-second Telegram poll uninterrupted throughout. |
| 2026-08-20 | **Plaintext secret sweep on VM 105 (§5.5).** A question about file permissions turned into an audit: `N8N_ENCRYPTION_KEY` existed in **six** files, five of them stale backups accumulated across four weeks of ordinary changes. All five removed after verifying each carried the same variable set as the live file. **One file now holds the key** — the irreducible minimum. The instructive one was `~/docker-compose-backup-20260721%.yml`: a month old, mode `664`, and **outside the stack directory**, so every check scoped to `~/n8n-stack/` had missed it. **The permission bit was a red herring** — `/home/leon` is `0750`, so "other" cannot traverse into it and the file mode never came into play; only `root` and `leon` have login shells. Tightening to `0600` would have been theatre. Two false positives were correctly excluded: `.bash_history` and `restore-verification.sh` mention the variable *name* but contain **zero occurrences of the value**, confirming the restore harness's 2026-08-12 design of reading the key into a variable and never storing it. `ansible/n8n.yml`'s local backup task was also removed — Tier 1 holds the file encrypted, off-host, with 14 versions, which beats a plaintext copy on the same disk as the thing it protects. Playbook re-verified `changed=0`. |
