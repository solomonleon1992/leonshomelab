...
**Wazuh is not captured, and the reason is specific.** SSH key access works. But every Wazuh configuration path — `/var/ossec/etc/*`, the indexer and dashboard configs — is root-only, and the backup account has no passwordless sudo (verified 2026-08-14). **That is a privilege decision, not a scripting problem** (§7.6), and the script deliberately does not work around it. Granting a backup job unattended root on the SIEM is a trade worth making consciously or not at all.
...
Verified run 2026-08-14: **22/22 items, 101 KB archive**, staging deleted, unlistable without the password.
