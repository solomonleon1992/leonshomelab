...
|---|---|---|---|
| n8n stack | ✅ Ansible playbook (volumes only) | ⚠️ compose holds secrets inline | ⚠️ hardened + verified 2026-08-20; restore-tested 2026-08-12 |
...
**Honest score as of 2026-08-20: Phase 2 complete — all six services are managed by Ansible. None has passed the rebuild test, which remains the open half.**
...
| 6 | n8n stack | VM 105 | ✅ **DONE 2026-08-20** — `ansible/n8n.yml`. Volumes externalised, state and credential decryption asserted. Secrets deliberately untouched; `changed=0` across three runs |
...
| 2026-08-20 | **§6 position 6 done — n8n. Phase 2 complete: all six services under Ansible.** The playbook never touches secrets, because this compose file holds `N8N_ENCRYPTION_KEY` inline; it edits only the volumes block, externalising the same directory-name fragility Caddy had. It asserts state counts identical before/after and that **credentials still decrypt** — the one check that proves the key survived. **The first attempt corrupted the live compose file.** A loose regexp let the end-of-line anchor match after the first volume key, so a second run replaced a fragment of the already-converted block and appended a duplicate, producing an unparseable file; only the already-running containers kept the stack up. **A loose pattern plus a non-idempotent replace is a corruption primitive**, and idempotency is not a nicety on a task that edits a secret-bearing file under a live job search. Two guards added, `changed=0` proven across three consecutive runs. Phase 2's remaining half is unchanged: **no service has passed the rebuild test** (§9.1 criterion 6) — converging a service in place is not reconstituting it on hardware that need not match. |
